...
3. **Sources — three layers, all merging:**
   ...
   - **Layer 3: Book-index reverse mode (two-tier)** — chapter-level topic annotations in `/sources/<book>/index.md`. Each chapter block carries a `**Topics:**` bulleted list (one topic per `- ` line) and, when curated, zero or more `- Use <ref> for <topic>` bullets. **The two surfaces have different force:**
     ...
     - **`- Use <ref> for <topic>` is hard.** A topic match is authoritative: parse `<ref>` (e.g. `2.5.4`) and read the section's range as canonical evidence. Line ranges live in the per-chapter file (split layout) or inline below the bullets (inline layout).
     Both can fire in the same run; `Use` matches read directly, `Topics`-only matches join the candidate list. Topics is generated by `source-indexer` (optionally course-aware); Use bullets are written exclusively by `accept-canonical-chapters` after explicit user approval.
...
- **Split layout** (≥500-line indexes or ≥10 chapters with subsections) — `index.md` carries only chapter blocks with `**Topics:**` bulleted lists and (when curated) `- Use <ref> for <topic>` bullets. Section headings + line ranges live in per-chapter files `/sources/<book>/index-ch<NN>.md` (NN zero-padded, e.g. `index-ch02.md`, `index-ch14.md`). For an explicit cite like `Huemer UK 2.5.4`: parse the leading `2` → open `index-ch02.md` → find `### 2.5.4 ... [<a>-<b>]` → read range. For reverse-mode (topic only): match against `Use` bullets (hard surface) for direct read; match against `**Topics:**` (soft surface) for candidate proposal — see Layer 3 above for the precedence.
...
| **accept-canonical-chapters** | "accept canonical chapters in &lt;book&gt;" (optionally with a course slug); "add canonical Use marks"; after ingest, ready to commit canonical-status claims | Approved `- Use <ref> for <topic>` bullets in `/sources/<book>/index.md`; with course, also `books:` and per-module `**Sources:**` updates in `/courses/<course>/index.md` |
...
Selection: philosophy always matches → essay-philosophy. Course housekeeping → prepare-course. Library housekeeping → ingestor (file the source) → indexer (Topics + structure) → accept-canonical-chapters (`Use` marks; course-MD when relevant) → accept-canonical-positions (the user's stances). Profile updates → voice-dna-creator / icp-creator.
