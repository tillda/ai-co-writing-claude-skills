...
3. **Sources — three layers, all merging:**
   ...
   - **Layer 3: Book-index reverse mode (two-tier)** — chapter-level topic annotations in `/sources/<book>/index.md`. Each chapter block carries a `**Topics:**` keyword line and, when curated, zero or more `- Use <ref> for <topic>` bullets. **The two surfaces have different force:**
